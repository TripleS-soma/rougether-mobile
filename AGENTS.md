...
- **공용 UI는 스토리 필수** (2026-09-16): `src/components/ui/`에 컴포넌트를 추가하거나 동작·상태를 바꾸면 `stories/components/<이름>.stories.tsx`도 추가·갱신합니다(Dev 갤러리 등록과 별개 — 스토리는 브라우저 카탈로그·CI 렌더/접근성/시각 회귀 테스트의 입력). 작성법·검증 명령은 [`docs/storybook.md`](docs/storybook.md).
...
- **포트폴리오 기록** (2026-09-16): 이 프로젝트는 개발자 포트폴리오에 싣는다. 작업·대화 중 **기술적 챌린지나 트러블슈팅**(원인을 좁혀야 했던 버그·빌드/배포 사고, 플랫폼 제약 우회, 위험한 대규모 변경의 설계 판단)이 생기면 **묻지 말고 판단해서** `docs/portfolio/`에 사례 파일을 쓰고 목록에 추가한다. 작성 기준·틀·금지 사항은 [`docs/portfolio/README.md`](docs/portfolio/README.md). 수치는 실측값만, 같은 사례의 후속은 기존 파일에 `## 후속`으로. 해당 작업 PR에 같이 싣거나, 이미 머지됐으면 `docs(portfolio):` PR로 따로 낸다.
