...
export type GachaMachine = {
  ...
  icon: PictogramName;
  accent: string;
  /** Selector row grouping — themed machines drop items, the rest characters. */
  kind: 'item' | 'character';
};
...
export function toGachaMachine(g: GachaResponse, index = 0): GachaMachine {
  return {
    ...
    // Item gachas draw from a theme; the character gacha has none.
    kind: g.themeId == null ? 'character' : 'item',
  };
}
