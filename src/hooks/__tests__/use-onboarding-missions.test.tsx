...
describe('useOnboardingMissions (#571)', () => {
  ...
  it('autoStart에 플래그가 없으면 1단계(루틴 완료)부터 시작한다 (#1324)', async () => {
    const { result } = await renderHook(() => useOnboardingMissions(true));
    await waitFor(() => expect(result.current.active).toBe(true));
    expect(result.current.stepIndex).toBe(0);
    expect(result.current.step?.id).toBe('complete-routine');
    expect(track).toHaveBeenCalledWith('onboarding_mission_start', { step: 'complete-routine' });
  });
   ...
   */
  it('첫 단계는 루틴 완료(#1324), 마지막 단계는 집에 친구 초대하기다 (#841)', () => {
  ...
  it('4단계까지 순서대로 완료하면 체인이 끝난다 (#841 → #1324)', async () => {
    const { result } = await renderHook(() => useOnboardingMissions(true));
    await waitFor(() => expect(result.current.active).toBe(true));
  ...
  it('현재 단계 완료만 진행되고, 순서가 다른 완료는 무시된다', async () => {
    ...
    await act(async () => result.current.complete('complete-routine'));
    expect(result.current.stepIndex).toBe(1);
    expect(result.current.step?.id).toBe('first-draw');
    // 완료 전환 시트용 index + 퍼널 이벤트.
    expect(result.current.completedIndex).toBe(0);
    expect(track).toHaveBeenCalledWith('onboarding_mission_complete', { step: 'complete-routine' });
  ...
  it('건너뛰기는 즉시 종료하고 스킵 플래그를 영속한다', async () => {
    ...
    expect(result.current.active).toBe(false);
    expect(track).toHaveBeenCalledWith('onboarding_mission_skip', { step: 'complete-routine' });
