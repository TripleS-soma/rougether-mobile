...
jest.mock('@/features/minigame/merge-html', () => ({
  createMergeHtml: jest.fn((config: unknown) => JSON.stringify(config)),
}));

describe('MergeGame lifecycle bridge', () => {
  ...
  it('pauses in the background, synchronizes readiness, and disposes the game on unmount', async () => {
    const { getByTestId, rerender, unmount } = await render(
      <MergeGame seed={42} onFinish={jest.fn()} />,
    );
    const game = getByTestId('merge-game');
    ...
    expect(mockInjectJavaScript).toHaveBeenLastCalledWith(
      'window.setMergeActive && window.setMergeActive(false); true;',
    );
    await act(async () => {
    ...
    await rerender(<MergeGame seed={42} active={false} onFinish={jest.fn()} />);
    await act(async () => appStateChanged('active'));
    expect(mockInjectJavaScript).toHaveBeenLastCalledWith(
      'window.setMergeActive && window.setMergeActive(false); true;',
    );
    await rerender(<MergeGame seed={42} active onFinish={jest.fn()} />);
    expect(mockInjectJavaScript).toHaveBeenLastCalledWith(
      'window.setMergeActive && window.setMergeActive(true); true;',
    );
    await unmount();
  ...
  it('rejects invalid messages and delivers a valid finish only once per seed', async () => {
    ...
    const { getByTestId, rerender } = await render(
      <MergeGame seed={42} onFinish={onFinish} onPauseChange={onPauseChange} />,
    );
    const game = getByTestId('merge-game');
    const { channelId } = JSON.parse(game.props.source.html);
    const result = {
    ...
    await rerender(<MergeGame seed={43} onFinish={onFinish} />);
    const nextGame = getByTestId('merge-game');
  ...
  it('keeps the document stable when parent callbacks or activity change', async () => {
    ...
    const source = getByTestId('merge-game').props.source;
    expect(getByLabelText('고양이 합치기')).toBeTruthy();
    expect(JSON.parse(source.html).allowManualTime).toBe(false);
    await rerender(<MergeGame seed={42} active={false} onFinish={jest.fn()} />);
    expect(getByTestId('merge-game').props.source).toBe(source);
    expect(createMergeHtml).toHaveBeenCalledTimes(1);
  });

  it('allows only the inline game document to navigate', async () => {
    const { getByTestId } = await render(<MergeGame seed={42} onFinish={jest.fn()} />);
    const shouldLoad = getByTestId('merge-game').props.onShouldStartLoadWithRequest;
