...
it.each(PLAYABLE_MINIGAMES)('shows $name one control hint for the selected game', async (game) => {
...
it('shows a server result only after confirmation and blocks duplicate actions during save', async () => {
  ...
  expect(ui.getByText('180점')).toBeTruthy();
  expect(ui.queryByText('게임')).toBeNull();
...
it('labels practice and provides a fresh practice attempt after game over', async () => {
  ...
  expect(ui.getByText('연습 · 기록 안 함')).toBeTruthy();
...
it('shows one recovery banner and retains the old score retry if a new start also failed', async () => {
  ...
  expect(ui.getByText('게임을 시작하지 못했어요')).toBeTruthy();
  expect(ui.getByText('이전 기록 저장 대기 중')).toBeTruthy();
