...
import { ContentMaxWidth, Radius, ShadowColor, Spacing } from '@/constants/theme';
import {
...
export function RoomDecorScreen({
  ...
  // 웹 데스크톱 2단 (#1230 후속) — 창 ≥ 960px에서만 true.
  ...
  // 캔버스·카탈로그는 2단과 1단이 같은 트리다 — 아래 분기는 배치만 정한다. 예전엔 두 분기가
  // 이 JSX를 따로 들고 있어(341줄) 한쪽만 고치면 웹 2단에서만 조용히 어긋났다(장부 2라운드 1번).
  const bodyStyle = [
    styles.body,
    headerInset,
    actionBarInset ? { paddingBottom: actionBarInset } : null,
  ];
  // 캔버스만 묶는다 — 방은 aspectRatio라 폭이 넓어지면 높이도 같이 커져 카탈로그가 화면
  // 밖으로 밀린다. 카탈로그는 반대로 폭을 다 써서 열을 늘린다(DecorGrid) — 한 번에 보이는
  // 가구가 많을수록 좋다 (#725).
  const canvasView = (
  ...
  return (
    <View style={[styles.screen, useScreenStyle([])]}>
      ...
      {/* 웹 데스크톱 2단 (#1230 후속): 왼쪽에 방 캔버스 고정, 오른쪽에 카탈로그 스크롤.
          폰·좁은 창은 종전대로 한 스크롤에 위아래로. */}
      {split ? (
        <View style={styles.splitRow} testID="decor-split">
          <View style={[styles.splitHero, headerInset]}>{canvasView}</View>
          <ScrollView style={styles.splitPanel} contentContainerStyle={bodyStyle}>
            {catalogBody}
          </ScrollView>
        </View>
      ) : (
...
const styles = StyleSheet.create({
  ...
  // 2단 (#1230 후속) — 왼쪽 방 칸(폰 컬럼 폭) + 오른쪽 카탈로그(콘텐츠 폭), 한 블록으로 중앙.
  splitRow: {
    flex: 1,
    flexDirection: 'row',
  ...
  splitApply: {
    width: '100%',
    maxWidth: APP_FRAME_MAX_WIDTH + Spacing.six + ContentMaxWidth,
    alignSelf: 'center',
  },
  floatBar: {
    position: 'absolute',
