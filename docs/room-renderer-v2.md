...
집 안 방은 배포된 프레임의 투명 구멍 좌표를 유지하고 프레임 표시 높이를 조정합니다. `stacked-v1-20260905`의 320×320 구멍은 320×384로 표시됩니다. `contentFit="fill"`을 사용하는 프레임만 세로로 늘어나며, 그 뒤에 그리는 방의 가구·캐릭터는 늘어나지 않습니다. 기존형 fallback도 구멍의 가로/세로 비율에 맞춰 표시 높이를 계산합니다. 집 선택 썸네일은 원본 비율을 유지합니다. CDN 파일·카탈로그 저장 키·새 집 화풍 시안은 변경하지 않습니다.
