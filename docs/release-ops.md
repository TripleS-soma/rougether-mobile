...
- **채널 지도 (2026-09-03 재배선)**: 채널은 바이너리에 박히고 브랜치는 EAS에서 바꿀 수 있다. 지금 지도는 다음과 같다 — `eas channel:view <name>`으로 확인.
  ...
  - `dev` 채널 → **`production` 브랜치** (2026-09-16 재배선): 1.5.3 빌드 125가 `testflight` 프로필(채널 dev) 상태로 **App Store 심사에 제출**돼 111과 같은 사고가 재발했다. 125·Android vc11 apk 등 dev 채널을 듣는 설치본은 이제 **스토어 레인**이다.
  - `internal` 채널 → `dev` 브랜치: **dev 레인의 현재 이름.** `eas.json`의 `preview`·`testflight` 프로필이 이 채널로 빌드되고, `eas-deploy`는 종전대로 `--branch dev`로 발행한다. 기존 테스터 기기는 다음 네이티브 윈도우의 새 빌드를 받아야 dev 레인으로 돌아온다.
  - **재발 방지**: ASC "빌드 추가"에서 testflight 프로필 빌드(채널이 production이 아닌 것)를 고르지 말 것. `eas build:list`의 `channel` 컬럼이 `production`인 빌드만 심사 제출.
...
- **머지 전 지정 버전의 iOS 후보만 만들기**: `store-build`의 기본 `mode=production`은 위 규칙 그대로다. 별도 `mode=testflight-build-only`는 지정 브랜치에서 `platform=ios`, `submit=false`, `expected_source_sha=<해당 브랜치의 전체 40자리 SHA>`, `expected_app_version=<app.json의 버전>`으로 실행한다(기본 버전은 과거 후보와 호환되는 1.5.0). 먼저 기본 `action=inspect`로 기존 CI의 `EXPO_TOKEN`을 이용해 최근 iOS 빌드 식별자·상태·버전·소스·런타임·프로필·채널과 원격 빌드 번호·로컬 지문만 확인한다(빌드 시 testflight 프로필 지문을 별도 확인하며, 토큰·키·산출물 URL은 출력하지 않음). 새 빌드가 승인된 경우에만 `action=build`로 다시 실행하며, 동일 SHA의 최신 `check`·`prebuild`·`ios-plist`·`android-manifest`·`release-smoke` 성공과 명시한 앱 버전과 testflight/dev/preview/store 설정을 요구한다. 2026-09-13 스피커 후보는 1.5.2이며 기존 115 제출 전용 경로는 변경하지 않는다. 원격 카운터의 다음 번호를 사용하고 선택 입력 `expected_ios_build`와도 대조한다. 같은 지문의 완료·진행·실패 시도가 있거나 조회가 불완전하면 새 빌드를 거부한다. `eas-deploy-dev`와 직렬화하며, iOS 빌드 1회 완료까지 기다릴 뿐 자동 제출·OTA·production 폴백·자격증명 수정·자동 재시도는 하지 않는다. **이 경로의 성공은 EAS 후보 바이너리 생성이지 TestFlight 배포 완료가 아니다.** 실제 제출과 서버 기능 활성화는 각각의 QA·승인 절차를 별도로 거친다.

- **완성된 115번 빌드만 TestFlight에 제출**: 명시 승인된 EAS 빌드 `3e9beb68-454f-44fd-883e-3b7a4408e184`(1.5.0/115, 앱 소스 `8496faddd40093b92f58da7fea91274fff219bea`)는 `store-build`의 별도 `mode=testflight-submit-only`를 사용한다. `existing_build_id`, `expected_source_sha`, `expected_ios_build=115`는 기존 바이너리를 가리키며, `expected_tooling_sha`는 제출 도구를 실행하는 현재 커밋으로 별개다. 기본 `action=inspect`는 EAS 빌드·제출 기록과 Apple의 현재 TestFlight 상태만 조회하며 제출하지 않는다. 업로드를 승인한 경우에만 `action=submit`, `submit=true`, `platform=ios`로 실행한다. 빌드 ID·버전·번호·앱 소스·런타임·프로필·채널·프로젝트·완료 상태와 기존 앱 소스 CI를 대조한 뒤 지정된 `EXPO_TOKEN`/`ASC_API_KEY_P8`를 Actions 내부에서만 사용한다. 개인키는 배타적으로 생성한 0600 임시 파일이며 정상·실패 경로와 `always()` 정리 단계에서 제거한다. 기존 제출 또는 Apple의 같은 버전·번호가 있으면 재제출하지 않는다. CLI 23.2.0의 정확한 `--id` 제출만 실행하고 TestFlight 그룹 자동 생성은 끈다. 재빌드·OTA·PR 병합·서버 활성화·App Store 심사 제출은 하지 않는다. 모호한 실패는 재실행하지 말고 제출 기록을 확인한다. EAS 업로드 완료 후 Apple 처리는 최대 15분 조회하며, 아직 처리 중이면 업로드 완료와 TestFlight 사용 가능 여부를 구분해서 보고한다.
...
- **후보 빌드와 실제 검증 소스를 일치시킨다**: 일반 PR CI는 움직이는 merge ref를 검사할 수 있다. 반면 `release-smoke`는 PR head SHA를 명시적으로 체크아웃하고 타입·전체 테스트·Release 빌드·첫 화면을 같은 소스에서 확인한다. 업로드된 `source-sha.txt`를 후보 SHA와 대조해야 하며, check-run의 `head_sha`만 보고 실제 checkout이 같다고 단정하지 않는다.
...
- **빌드 성공은 실행 가능을 뜻하지 않는다 — 제출 전 실행 확인은 필수** (2026-08-20, #915): 1.2.0 빌드 47은 EAS 빌드 성공·`check` 통과(테스트 990개)·ASC 업로드 성공을 전부 거치고도 **스플래시에서 즉사**했다(#913). 통과한 신호들 중 어느 것도 "앱이 켜지는가"를 보지 않았다.
  ...
  - **제출 전 실기기/시뮬레이터 실행 1회는 사람이 한다.** 로컬 iOS 빌드가 되므로(RNFB 26 #1031 이후 별도 레시피 없이 `npx expo prebuild --platform ios --clean` → `SENTRY_DISABLE_AUTO_UPLOAD=true npx expo run:ios --configuration Release` — Firebase는 SPM(dynamic frameworks, #1053)으로 들어오므로 별도 Podfile 주입은 없다(CocoaPods 배포 중단 2026-10 대비). **Debug 구성은 링크가 깨진다**: 미리 컴파일된 RN 코어가 Release용이라 `Sealable`·Hermes 인스펙터 심볼이 없다) EAS 쿼터 없이 시뮬레이터에서 띄워볼 수 있다. 스토어 제출·TestFlight 배포 전에 **앱을 실제로 켜서 첫 화면까지 확인**할 것.
