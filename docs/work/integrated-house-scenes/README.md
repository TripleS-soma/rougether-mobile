...
- `HouseSceneArtwork`: 같은 스타일의 환경 전용 backdrop을 `cover`로 채우고 원본 통합 그림을 균일 축척으로 올립니다. SVG alpha mask는 보호영역 안을 완전히 보존하고 바깥 근접 환경만 최대 원본 64px / 화면 24px에 걸쳐 흐리게 연결합니다. 주변 나무나 그림을 늘리는 9-slice는 사용하지 않습니다.
- `house-scenes/backdrops/sources.ts`: 네 테마의 로컬 환경 배경입니다. 장면과 환경은 Expo Image 캐시·사전 로딩을 사용합니다. SVG Image에는 `onError`가 없어 같은 원본의 Expo Image probe와 실제 SVG의 `onLoad`를 함께 확인합니다. 실제 SVG가 15초 안에 성공을 알리지 않으면 그림과 좌표를 함께 기존 프레임으로 돌립니다.
