...
세로 방 적용 후 6인 프레임을 화면 폭 100%로 그리면 헤더와 하단 탭까지 포함한 콘텐츠가 첫 화면보다 길어졌다. 6인 프레임에 한해 레벨·멤버 필을 지붕 옆에 띄우고, 프레임 아래 여백을 64px에서 16px로 줄인다. 화면과 헤더를 실제 측정한 높이에서 하단 탭 및 여백을 제외해 프레임 최대 폭을 정한다. 프레임·창문 비율과 저장 좌표는 바뀌지 않는다. 2·4인 및 legacy 프레임은 기존 폭을 유지하고, 가로 화면은 스크롤을 유지한다.

`/dev?entry=HouseSixSeatViewport&viewport=1`은 갤러리 카드의 패딩·중첩 스크롤 없이 실제 HouseScreen을 전체 화면에 놓는다. safe-area 59/34px fixture이며 헤더와 하단 탭의 여유를 포함한다. 테스트 데이터만 사용한다.

| 웹 뷰포트 | 프레임 위쪽 | 프레임 아래쪽 | 아래층 방 하단 | 초기 스크롤 |
| --------- | ----------- | ------------- | -------------- | ----------- |
| 320×568   | 157px       | 438px         | 403px          | 0px         |
| 393×852   | 157px       | 722px         | 652px          | 0px         |
| 430×932   | 157px       | 802px         | 723px          | 0px         |

세 크기에서 ScrollView의 콘텐츠 높이와 뷰포트 높이가 같아 최초 화면에 프레임 전체가 들어간다. 이 결과는 브라우저의 실제 렌더 측정이며 네이티브 기기 수신·실행 확인과 구분한다. 회귀 테스트는 화면 크기, 헤더 높이 변화, 2·4인 및 legacy 복귀, 가로 회전을 검증한다.
